...
# Trusted Indian news sources on NewsAPI
TRUSTED_SOURCES = [
    "the-times-of-india",
    "the-hindu",
    "bbc-news",
...
def search_news(query_text):
    ...
    try:
        ...
        for article in articles:
            ...
            # Only include if 40%+ similar
            if similarity > 0.4:
                log(f"✓ {source_name}: {similarity:.0%} match - {title[:50]}...")
                verified.append({
                    'source': source_name,
                    'link': url,
        ...
        log(f"✅ Verified: {len(verified)} matching articles")
        return verified
